...
class Smoke(torch.nn.Module):
    def __init__(self, probability: float = .5, path: str = None, type_: str = 'int32'):
        ...
        path = path if path else os.path.join('..', 'resources', 'smoke')
        self.smoke = [self.totensor(Image.open(os.path.join(path, file))) for file in os.listdir(path)]
    ...
    def forward(self, inp: torch.Tensor):
        ...
        ----------
        inp : torch.Tensor
            The input array which needs some smoke.
        ...
        if torch.rand(1) <= self.probability:
            overlap = self.smoke[torch.randint(0, len(self.smoke), (1,))]
...
if __name__ == '__main__':
    ...
    augmentation= Smoke(probability=1, type_='float32')
    f,a = plt.subplots(1,2)
    a[0].imshow(image.permute(1,2,0))
    a[1].imshow(augmentation(image).permute(1,2,0))
