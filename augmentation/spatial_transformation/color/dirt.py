...
class PerlinNoise1(torch.nn.Module):
    ...
    def forward(self, inp):
        ...
        if torch.rand(1) <= self.probability:
            ...
            *sizes, height, width = inp.shape
            ...
            noise = self.generate_fractal_noise_2d(shape, octaves, lacunarity, persistence)[...,:height, :width]
            noise = (torch.tensor(noise).to(device)*ins.max()).clamp(*clamp_params)
             
            rgb_fusion = (self.alpha * ins) + ((1 - self.alpha) * noise)
            inp = rgb_fusion.type(ins.type())
