...
class MultiplicativeGaussianNoise(torch.nn.Module):
    def __init__(self, probability: float = 1., mean: float = 0, std: float = 1.):
        """
        Augment the image with multiplicative gaussian noise.
...
class MultiplicativeWhiteNoise(torch.nn.Module):
    def __init__(self, probability: float = 1., amplitude: float = 1):
        """
        Augment the image with multiplicative white noise.
...
if __name__ == '__main__':
    ...
    augmentation = MultiplicativeGaussianNoise(probability=1, mean = 1, std = 0.3)
    augmentation = MultiplicativeWhiteNoise(probability=1, amplitude = 2)
